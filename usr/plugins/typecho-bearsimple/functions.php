    ...
    foreach ($emo as $v){
        if($v['type'] == 'image'){
            foreach ($v['container'] as $vv){
                $comment = str_replace($vv['data'], '<img width="30" height="30" src='.$dir.'/assets/owo/'.$vv['icon'] .''.'>', $comment);
            }
        }
    }
    return $comment;
...
?>
    ...
    <script src="<?php Helper::options()->themeUrl('assets/manage/bearui.min.js?t=1.3') ?>"></script>
...
<?php require_once('core/backup.php'); ?>
    ...
    $form->addInput($SliderOthers->multiMode());
     ...
     $SliderPics = new Typecho_Widget_Helper_Form_Element_Textarea(
          'SliderPics', NULL, "",
          '幻灯片图片', '格式:https://xxx.com/xxx.jpg|https://xxx.com/xxx.jpg，使用|作为分隔<br><font color="red">请先在图片设置-图片剪裁设置中设置允许的域名，否则前台幻灯片图片不显示</font>');
...
//4
 ...
 $CCFirewall = new Typecho_Widget_Helper_Form_Element_Select('CCFirewall', array('1' => '开启简易CC防护', '2' => '关闭简易CC防护'), '3', '选择是否开启简易CC防护', '若选择开启,可能会存在误拦截!');
...
$cutimage_domain = new Typecho_Widget_Helper_Form_Element_Textarea('cutimage_domain', NULL, $options->siteUrl, _t('图片剪裁域名限制'),'填写图片剪裁功能可用的域名，<font color="red">考虑到安全因素，本功能强制开启，非本项内所填域名无法使用剪裁功能</font><br>域名无需加http(s)://和后斜杠，例如baidu.com，多个域名使用英文逗号分隔');
 ...
 if (!empty($options->cutimage_domain)){
        $file = dirname(__FILE__).'/modules/BearThumb/Domain.dm';
        if(false!==fopen($file,'w+')){
            file_put_contents($file,$options->cutimage_domain);
        }
}
...
//区块设置
$module = new Typecho_Widget_Helper_Form_Element_Select('lastcomment', array('0' => '关闭右侧最新回复区块',  '1' => '开启右侧最新回复区块'), '0', '是否开启右侧最新回复区块', '开启后网站侧边栏将增加最新回复区块，显示最新的五条评论。');
...
'',_t('第一个标签对应的豆瓣图书ID'), _t('在此填写豆瓣图书中的图书ID，多本书使用英文逗号分隔，他将显示在前台我的书架页面中的第一个书架标签内'));
...
'',_t('第二个标签对应的豆瓣图书ID'), _t('在此填写豆瓣图书中的图书ID，多本书使用英文逗号分隔，他将显示在前台我的书架页面中的第二个书架标签内'));
...
'',_t('第三个标签对应的豆瓣图书ID'), _t('在此填写豆瓣图书中的图书ID，多本书使用英文逗号分隔，他将显示在前台我的书架页面中的第三个书架标签内'));
...
if (!empty($options->bilibili_accountid)){
        $file = dirname(__FILE__).'/vendors/Bilibili/Username.acc';
        if(false!==fopen($file,'w+')){
            file_put_contents($file,$options->bilibili_accountid);
        }
}

//打赏设置新增选择打赏样式
$Reward_style = new Typecho_Widget_Helper_Form_Element_Select('Reward_style', array('1' => '花里胡哨风格',  '2' => '白色风格','3'=>'简约风格'), '1', '打赏样式', '选择文章内打赏组件的风格样式');
    $Reward_style->setAttribute('class', 'bearui_content bearui_reward');
    $form->addInput($Reward_style->multiMode());
...
<div class="ui huge list">
  <div class="item">
    <div class="header" style="margin-bottom:3px">1、登录、回复可见</div>
    {bs-hide}隐藏内容{/bs-hide}或者[bs-hide]隐藏内容[/bs-hide]
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:3px">2、Todolist</div>
    已完成：{bs-todo type=true}已完成的内容{/bs-todo}<br>
    未完成：{bs-todo type=false}未完成的内容{/bs-todo}
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:3px">3、代码高亮</div>
    ```语言名称<br>
    代码内容<br>
    ```
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:3px">4、字体颜色</div>
   {bs-font color="Hex颜色代码"}内容{/bs-font}<br>
   Hex颜色代码例如#FFFFFF
  </div>
  <div class="item">
  <div class="header" style="margin-bottom:3px">5、iframe</div>
...
layui.use(['upload', 'element', 'layer'], function(){
  ...
  ,uploadListIns = upload.render({
    elem: '#uploadimages'
    ,url: '/usr/themes/bearsimple/upload/upload_img.php'
...
,before: function(obj){
    ...
    ,multiple: true
    ,auto: false
    ,data: {token: value,domain: domain}
